...
class ChatGroq(BaseInference):
    ...
    def invoke(self, messages: list[BaseMessage], json: bool = False) -> AIMessage:
        ...
        logger.info(termcolor.colored(f"Invoking Groq API with model: {self.model}", "magenta"))
        ...
        try:
            ...
            err_object = loads(err.response.text) if hasattr(err, 'response') and err.response.text else {}
            error_message = err_object.get('error', {}).get('message', str(err))
            print(f'\nError: {error_message}\nStatus Code: {err.response.status_code if hasattr(err, "response") else "Unknown"}')
            raise
    ...
    def stream(self, messages: list[BaseMessage],json=False)->Generator[str,None,None]:
        ...
        temperature=self.temperature
        url=self.base_url or "https://api.groq.com/openai/v1/chat/completions"
        logger.info(termcolor.colored(f"Streaming Groq API with model: {self.model}", "magenta"))
        messages=[message.to_dict() for message in messages]
        payload={
            "model": self.model,
            "messages": messages,
